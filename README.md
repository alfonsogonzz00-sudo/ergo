...
**Flujo:** grabar pantalla → abrir ERGO → elegir categoría → PLAY → animación de selección → aparece el reto → TE TOCA → EMPEZAR → temporizador → LO HE HECHO.
...
```
ergo/
├── ergo.html              ← toda la app (HTML, CSS y JS en un único archivo)
├── manifest.json            ← metadatos de la PWA (nombre, iconos, colores)
├── sw.js                    ← service worker (permite instalarla y abrirla sin conexión)
├── vercel.json              ← hace que la URL raíz cargue ergo.html automáticamente
├── icon-192.png
├── icon-512.png
...
Todos los archivos deben subirse juntos, en la misma carpeta — `manifest.json` y `sw.js` referencian los iconos y `ergo.html` con rutas relativas.

> **Si vienes de la versión anterior (INERGO):** el archivo principal se llamaba `inergo.html` y ahora se llama `ergo.html`. Borra `inergo.html` de tu repositorio de GitHub para que no quede suelto junto al nuevo, y revisa que `manifest.json`, `sw.js` y `vercel.json` sean estas versiones nuevas (cambiaron todos por dentro, aunque el nombre de archivo sea el mismo).

## Ejecutar en local

No requiere servidor ni instalación para probar la app en el navegador:

1. Descarga toda la carpeta `ergo` (no solo `ergo.html`).
2. Haz doble clic sobre `ergo.html` para abrirlo, o arrástralo a una pestaña.
...
# abre http://localhost:8000/ergo.html en el navegador
# o, desde el móvil en la misma red: http://TU_IP_LOCAL:8000/ergo.html
```
...
4. Aparecerá un icono de ERGO en tu pantalla de inicio; al abrirlo, se abre a pantalla completa, sin la barra de Safari.
...
3. Si tu código está en GitHub: importa el repositorio que contiene la carpeta `ergo`. Si no usas Git, pulsa la pestaña que permite **subir una carpeta directamente** (Vercel CLI, ver abajo, es la vía más rápida sin Git).
...
5. Pulsa **Deploy**.
6. En unos segundos obtendrás una URL del tipo `https://tu-proyecto.vercel.app`. Gracias al archivo `vercel.json`, esa URL raíz carga automáticamente `ergo.html` sin que tengas que escribir nada más al final. Al ser `https://`, el service worker funciona automáticamente y la app ya se puede instalar.
...
3. **Deploy manual:** arrastra la carpeta `ergo` completa (con todos sus archivos, no solo `ergo.html`) al recuadro de subida.
...
6. **Importante en Netlify:** el archivo `vercel.json` no le sirve a Netlify (es específico de Vercel). Si despliegas aquí, la URL raíz no cargará `ergo.html` sola — tendrás que compartir la URL completa `https://tu-sitio.netlify.app/ergo.html`, o renombrar `ergo.html` a `index.html` solo para este despliegue.
...
- **PWA (Progressive Web App):** `manifest.json` define nombre, iconos y colores; `sw.js` es el service worker que cachea la app para que abra incluso sin conexión una vez visitada por primera vez. Solo funciona instalada/servida por `https://` (o `localhost`); no funciona abriendo `ergo.html` directamente con doble clic.
