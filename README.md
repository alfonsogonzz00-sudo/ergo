...
Todos los archivos deben subirse juntos, en la misma carpeta — `manifest.json` y `sw.js` referencian los iconos con rutas relativas.

## Ejecutar en local

No requiere servidor ni instalación para probar la app en el navegador:

1. Descarga toda la carpeta `inergo` (no solo `index.html`).
2. Haz doble clic sobre `index.html` para abrirlo, o arrástralo a una pestaña.
...
# o, desde el móvil en la misma red: http://TU_IP_LOCAL:8000
```
...
5. Pulsa **Deploy**.
6. En unos segundos obtendrás una URL del tipo `https://tu-proyecto.vercel.app`. Al ser `https://`, el service worker funciona automáticamente y la app ya se puede instalar.
...
3. **Deploy manual:** arrastra la carpeta `inergo` completa (con todos sus archivos, no solo `index.html`) al recuadro de subida.
...
6. **Dominio propio (opcional):** en el sitio, ve a **Site configuration → Domain management → Add a domain**, añade tu dominio y sigue las instrucciones DNS que te indique Netlify.
...
- **PWA (Progressive Web App):** `manifest.json` define nombre, iconos y colores; `sw.js` es el service worker que cachea la app para que abra incluso sin conexión una vez visitada por primera vez. Solo funciona instalada/servida por `https://` (o `localhost`); no funciona abriendo `index.html` directamente con doble clic.
